...
from
	(
		select *, rank() over(partition by userid, courseid, assetid order by created)
		from user_progressed_lecture
		where total >1 and created < current_date and created > (select max(date) from user_daily_course_consumption)
	)
